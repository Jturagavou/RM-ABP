...
class CollaborationManager: ObservableObject {
    ...
    func createGroup(name: String, description: String, creatorId: String) async throws -> AccountabilityGroup {
        ...
        groupWithCreator.members.append(GroupMember(userId: creatorId, role: .admin))
        
        try await db.collection("groups").document(group.id).setData(groupWithCreator.toFirestoreData())
